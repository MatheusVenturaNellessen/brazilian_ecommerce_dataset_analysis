...
|-------|-----------|--------------|----------|--------|---------------|------------|---------------|
| `order_id` | Identificador único do pedido | STRING | Não |  | PK |  |  |
| `customer_id` | Chave para o conjunto de dados do cliente. Cada pedido possui um `customer_id` único | STRING | Não |  | FK | `customers.customer_id` | customers (1:N) orders |
| `order_status` | Referência ao status do pedido (entregue, enviado, etc.) | STRING | Não |  |  |  |  |
| `order_purchase_timestamp` |  Exibe o registro de data e hora da compra | DATETIME | Não |  |  |  |  |
| `order_approved_at` | Exibe o registro de data e hora da aprovação do pagamento | DATETIME | Sim | 0.16|  |  |  |
| `order_delivered_carrier_date` | Exibe o registro de data e hora da postagem do pedido. Indica quando ele foi entregue ao parceiro logístico | DATETIME | Sim | 1.79 |  |  |  |
| `order_delivered_customer_date` | Mostra a data real de entrega do pedido ao cliente | DATETIME | Sim | 2.9 |  |  |  |
| `order_estimated_delivery_date` | Exibe a data de entrega estimada que foi informada ao cliente no momento da compra | DATETIME | Não |  |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência |  Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|----------------|
| `order_id` | Identificador único do pedido | STRING | Não |  | PK (composta) e FK | `orders.order_id` | orders (1:N) order_items |
| `order_item_id` | Número sequencial que identifica a quantidade de itens incluídos no mesmo pedido | INTEGER | Não |  | PK (composta) |  |  |
| `product_id` | Identificador único do produto | STRING | Não |  | FK | `products.product_id` | products (1:N) order_items | 
| `seller_id` | Identificador único do vendedor | STRING | Não |  | FK | `sellers.seller_id` | sellers (1:N) order_items |
| `shipping_limit_date` | Mostra a data limite de envio do vendedor para que ele entregue o pedido ao parceiro logístico | DATETIME | Não |  |  |  |  |
| `price` | Preço do item | NUMERIC | Não |  |  |  |  |
| `freight_value` | Valor do frete por item (se um pedido tiver mais de um item, o valor do frete será dividido entre os itens) | NUMERIC | Não |  |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência |  Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|----------------|
| `order_id` | Identificador único de um pedido | STRING | Não |  | PK (composta) e FK |  `orders.order_id` | orders (1:N) order_payments |
| `payment_sequential` | Um cliente pode pagar um pedido com mais de um método de pagamento. Nesse caso, será criada uma sequência para processar todos os pagamentos | INTEGER | Não |  | PK (composta) |  |  |
| `payment_type` | Método de pagamento escolhido pelo cliente | STRING | Não |  |  |  |  |
| `payment_installments` | Número de parcelas escolhido pelo cliente | INTEGER | Não |  |  |  |  |
| `payment_value` | Valor da transação | NUMERIC | Não |  |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência |  Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|----------------|
| `review_id` | Identificador único de avaliação | STRING | Não |  | PK (composta) |  |  |
| `order_id` | Identificador único do pedido | STRING | Não |  |  PK (composta) e FK | `orders.order_id` | orders (1:0..1) order_reviews |
| `review_score` | Nota de 1 a 5 dada pelo cliente em uma pesquisa de satisfação | INTEGER | Não |  |  |  |  |
| `review_comment_title` | Título do comentário deixado pelo cliente | STRING | Sim | 88.3 |  |  |  |
| `review_comment_message` | Mensagem de comentário da avaliação deixada pelo cliente | STRING | Sim | 58.7 |  |  |  |
| `review_creation_date` | Mostra a data em que a pesquisa de satisfação foi enviada ao cliente | DATETIME | Não |  |  |  |  |
| `review_answer_timestamp` | Exibe o registro de data e hora da resposta da pesquisa de satisfação | DATETIME | Não |  |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência |  Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|----------------|
| `product_id` | Identificador único do produto | STRING | Não |  | PK |  |  |
| `product_category_name` | Categoria do produto | STRING | Sim | 1.8 | FK | `product_category_name_translation.product_category_name` | product_category_name_translation (1:N) products |
| `product_name_lenght` | Número de caracteres extraídos do nome do produto | INTEGER | Sim | 1.8 |  |  |  |
| `product_description_lenght` | Número de caracteres extraídos da descrição do produto | INTEGER | Sim | 1.8 |  |  |  |
| `product_photos_qty` | Número de fotos de produtos publicadas | NUMERIC | Sim | 1.8 |  |  |  |
| `product_weight_g` | Peso do produto medido em gramas | NUMERIC | Sim | 0.006 |  |  |  |
| `product_length_cm` | Comprimento do produto medido em centímetros | NUMERIC | Sim | 0.006 |  |  |  |
| `product_height_cm` | Altura do produto medido em centímetros | NUMERIC | Sim | 0.006 |  |  |  |
| `product_width_cm` | Largura do produto medida em centímetros | NUMERIC | Sim | 0.006 |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência | Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|---------------|
| `customer_id` | Chave para o conjunto de dados de pedidos. Cada pedido possui um `customer_id` único | STRING | Não |  | PK |  |  |
| `customer_unique_id` | Identificador único de um cliente | STRING | Não |  | Business Key | customers.costumer_unique_id (N:1) customers.customer_id |  |
| `customer_zip_code_prefix` | Primeiros cinco dígitos do CEP do cliente | STRING | Não |  |  | Pode ser associado a `geolocation.geolocation_zip_code_prefix` | customers (1:1*) geolocation |
| `customer_city` | Nome da cidade do cliente | STRING | Não |  |  |  |  |
| `customer_state` | Unidade Federativa do cliente | STRING | Não |  |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência | Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|---------------|
| `seller_id` | Identificador único do vendedor | STRING | Não |  | PK |  |  |
| `seller_zip_code_prefix` | Primeiros 5 dígitos do CEP do vendedor | STRING | Não |  |  | Pode ser associado a `geolocation.geolocation_zip_code_prefix` | sellers (1:1*) geolocation |
| `seller_city` | Nome da cidade do vendedor | STRING | Não |  |  |  |  |
| `seller_state` | Unidade Federativa do vendedor | STRING | Não |  |  |  |  |
...
| Campo | Descrição | Tipo de dado | Nullable | % Null | Tipo de chave | Referência | Cardinalidade |
|-------|-----------|--------------|----------|--------|---------------|------------|---------------|
| `geolocation_zip_code_prefix` | Primeiros 5 dígitos do código postal | STRING | Não | 0 |  | Pode ser associado a `customers.customer_zip_code_prefix` e `sellers.seller_zip_code_prefix` | geolocation_zip_code_prefix (1*:1) customers.customer_zip_code_prefix e sellers.seller_zip_code_prefix |
| `geolocation_lat` | Latitude | NUMERIC | Não | 0 |  |  |  |
| `geolocation_lng` | Longitude | NUMERIC | Não | 0 |  |  |  |
